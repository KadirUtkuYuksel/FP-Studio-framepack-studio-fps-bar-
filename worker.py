...
def worker(
    ...
    seed, 
    total_second_length, 
    fps_selection,       # NEW: FPS selection parameter
    latent_window_size,
    steps, 
    cfg, 
    gs, 
    ...
    # Calculate total_latent_sections based on FPS selection
    # Base calculation: for 30 FPS, we want (total_second_length * 30) / (latent_window_size * 4)
    # For other FPS, scale the numerator by fps_selection
    total_latent_sections = (total_second_length * fps_selection) / (latent_window_size * 4)
    ...
    try:
        ...
        # Set FPS for the generator
        studio_module.current_generator.fps = fps_selection
        ...
        # Update the generator's FPS attribute
        # Note: latent_window_size should NOT be scaled by FPS - it determines step count
        # FPS only affects video duration, not the number of generation steps
        studio_module.current_generator.fps = fps_selection
        ...
        def callback(d):
            ...
            if model_type == "Video":
                ...
                # Use fps_selection instead of hardcoded 30 FPS
                input_video_time = input_video_frame_count * 4 / fps_selection  # Convert latent frames to time
                current_pos = input_video_time + (total_generated_latent_frames * 4 - 3) / fps_selection
                # Original position is the remaining time to generate
                original_pos = total_second_length - (total_generated_latent_frames * 4 - 3) / fps_selection
            else:
                # For other models, calculate as before
                current_pos = (total_generated_latent_frames * 4 - 3) / fps_selection
        ...
        for i_section_loop, latent_padding in enumerate(latent_paddings): # Existing loop structure
            ...
            save_bcthw_as_mp4(history_pixels, output_filename, fps=fps_selection, crf=settings.get("mp4_crf"))
