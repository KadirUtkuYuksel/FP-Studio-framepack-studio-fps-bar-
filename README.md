...
### Full Installation (ZIP → Frampack Studio → Unzip → Run)
1) Download the ZIP of this project to your computer.
2) Copy the ZIP into your Frampack Studio folder.
3) Unzip the contents inside the Frampack Studio folder.
4) Open the unzipped folder and run the batch script:
```powershell
.\n+.\n+.
.
.
...
Manual alternative (no PowerShell):
- Right-click the ZIP → Extract All… into your Frampack Studio folder
- Double-click `apply_fps_features.bat` in the extracted folder

Automatic alternative (PowerShell):
```powershell
# Replace paths as needed
$zipPath = "C:\\Path\\To\\fps_feature_files.zip"
$studioPath = "C:\\Path\\To\\Frampack Studio"

Expand-Archive -Path $zipPath -DestinationPath $studioPath -Force

# If the ZIP extracts into a subfolder, change into it, then run the batch
Set-Location (Join-Path $studioPath "fps_feature_files")
.\n+.
.
.
.
...
Notes:
- The batch file name in this repo is `apply_fps_features.bat`. If your environment uses a shorter alias like `applyfps.bat`, run that instead.
- To revert changes later, use `restore_backup.bat` in the same folder.
